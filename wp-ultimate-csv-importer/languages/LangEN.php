<?php
/******************************************************************************************
 * Copyright (C) Smackcoders. - All Rights Reserved under Smackcoders Proprietary License
 * You can contact Smackcoders at email address info@smackcoders.com.
 *******************************************************************************************/

namespace Smackcoders\UCI\Core;

if ( ! defined( 'ABSPATH' ) )
exit; // Exit if accessed directly

class LangEN {
        private static $en_instance = null , $media_instance;

        public static function getInstance() {
                if (LangEN::$en_instance == null) {
                        LangEN::$en_instance = new LangEN;
                        return LangEN::$en_instance;
                }
                return LangEN::$en_instance;
        }

        public static function contents(){
                $response = array('ImportUpdate' => 'Import / Update',
                'SelectAllImages' => 'Select all images in your import file to avoid import failures',
                'ChooseImagesToImport' => 'Choose images to import. Utilize the checkbox below to select all images.',
                'FileName' => 'File Name:',
                'OK' => 'OK',
                'Exportfiltereddata' => 'Export filtered data',
                'Exportfiltereddatadesc' => 'Lets you get only required data with the different advanced filters',
                'Backupineditableformat' => 'Backup in editable format',
                'Backupineditableformatdesc' => 'Backup in 4 different file formats like CSV, XML, JSON,XLS.',
                'AutoScheduledBackups' => 'Auto Scheduled Backups',
                'AutoScheduledBackupsdesc' => 'Scheduled export helps backup as editable text file format in regular interval.',
                'AutoSchedulewithreusabletemplates' => 'Create advanced import rules & conditional logic',
                'Dashboard' => 'Dashboard',
                'Manager' => 'Manage',
                'csv_importlink' => 'click here',
                'Export' => 'Export',
                'Settings' => 'Settings',
                'Support' => 'Support',
                'OrderMetaFields' => 'Order Meta Fields',
                'UploadfromDesktop' => 'Upload from Desktop',
                'UploadfromFTPSFTP' => 'Upload from FTP / SFTP',
                'UploadfromFTP' => 'Upload from FTP',
                'UploadfromURL' => 'Upload from URL',
                'ChoosFileintheServer' => 'Choose File in the Server',
                'DragDropyourfilesor' => 'Drag & Drop your files or',
                'Browse' => 'Browse',
                'NewItem' => 'New Item',
                'ExistingItems' => 'Existing Items',
                'ImportEachRecordAs'=> 'Import each record as',
                'Continue' => 'Continue',
                'Search' => 'Search',
                'FromDate' => 'From Date',
                'ToDate' => 'To Date',
                'SEARCH' => 'SEARCH',
                'AccessKey' => 'AccessKey',
                'SavedTemplate' => 'Saved Template',
                'TEMPLATES' => 'TEMPLATES',
                'MATCHEDCOLUMNSCOUNT' => 'MATCHED COLUMNS COUNT',
                'MODULE' => 'MODULE',
                'CREATEDTIME' => 'CREATED TIME',
                'ACTION' => 'ACTION',
                'USETEMPLATE' => 'USE TEMPLATE',
                'CREATENEWMAPPING' => 'CREATE NEW MAPPING',
                'BACK' => 'BACK',
                'ADVANCEDMODE' => 'ADVANCED MODE',
                'DRAGDROPMODE' => 'DRAG & DROP MODE',
                'WordpressFields' => 'Wordpress Fields',
                'WPFIELDS' => 'WP Fields',
                'CSVHEADER' => 'CSV Header',
                'Action' => 'Action',
                'Name' => 'Name',
                'HINT' => 'HINT',
                'Example' => 'Example',
                'newtabfailedMedia' => 'Media',
                'CouponMetaFields' => 'Coupon Meta Fields',
                'MediaFields' =>'Media Fields',
                'WordPressCoreFields' => 'WordPress Core Fields',
                'FeaturedFields' => 'Featured Image Meta',
                'ACFFreeFields' => 'ACF Free Fields',
                'ACFFields' => 'ACF Fields',
                'ACFGroupFields' => 'ACF Group Fields',
                'ACFProFields' => 'ACF Pro Fields',
                'ACFRepeaterFields' => 'ACF Repeater Fields',
                'ACFFlexibleFields' => 'ACF Flexible Fields',
                'TypesCustomFields' => 'Types Custom Fields',
                'PodsFields' => 'Pods Fields',
                'JobListingFields' => 'Job Listing Fields',
                'CustomFieldSuite' => 'Custom Field Suite',
                'AllInOneSeoFields' => 'All In One Seo Fields',
                'MetaBoxFields' => 'Meta Box Fields',
                'YoastSeoFields' => 'Yoast Seo Fields',
                'SlimSeoFields' => 'Slim SEO Fields',
                'EDDDownloadsFields' => 'EDD Downloads Fields',
                'EDDCustomersFields' => 'EDD Customers Fields',
                'EDDDiscountsFields' => 'EDD Discounts Fields',

                'SureCartProductsFields' =>  "SureCart Products Fields",
                'SureCartCustomersFields'=> "SureCart Customers Fields",
                'SureCartCouponsFields'=> "SureCart Coupons Fields",

                'ListeoFields' => 'Listeo Fields',
                'WPMLFields' => 'WPML Fields',
                'JetEngineFields' => 'Jet Engine Fields',
                'JetEngineRFFields' => 'Jet Engine Repeater Fields',
                'JetEngineCPTFields' => 'Jet Engine CPT Fields',
                'jetEngineCPTRFFields' => 'Jet Engine CPT RF Fields',
                'JetEngineCCTFields' => 'Jet Engine CCT Fields',
                'JetEngineCCTRFFields' => 'Jet Engine CCT Repeater Fields',
                'jetEngineTaxonomyFields' => 'Jet Engine Taxonomy Fields',
                'jetEngineTaxonomyRFFields' => 'Jet Engine Taxonomy Repeater Fields',
                'JetEngineRelationsFields' => 'Jet Engine Relations Fields',
                'JetEngineBookingFields' => 'Jet Engine Booking Fields',
                'RankMathFields'=>'Rank Math Fields',
                'RankMathProFields'=>'Rank Math Pro Fields',
                'SeoPressFields' => 'SEOPress Fields',
                'ElementorFields' => 'Elementor Fields',
                'TotalPressFields' => 'TotalPress Fields',
                'replyattributesfields' => 'Reply Attributes Fields',
                'forumattributesfields' => 'Forum Attributes Fields',
                'topicattributesfields' => 'Topic Attributes Fields',
                'BillingAndShippingInformation' => 'Billing and Shipping Information',
                'CustomFieldsWPMemberFields' => 'Custom Fields WP Member Fields',
                'CustomFieldsMemberFields' => 'Custom Fields Member Fields',
                'ProductMetaFields' => 'Product Meta Fields',                
                'ProductAttrFields' => 'Product Attribute Fields',
                'ProductBundleMetaFields' => 'Product Bundle Meta Fields',
                'PPOMMetaFields' => 'PPOM Meta Fields',
                'EventCalendarMetaFields' => 'EventCalendar Meta Fields',
                'EPOMetaFields' => 'EPO Meta Fields',
                'WCPAMetaFields' => 'WCPA Meta Fields',
                'ListingMetaFields' => 'Listing Fields',
                'WPECommerceCustomFields' => 'WP ECommerce Custom Fields',
                'EventsManagerFields' => 'Events Manager Fields',
                'CMB2CustomFields' => 'CMB2 Custom Fields',
                'CourseSettingsFields' => 'Course Settings Fields',
                'CurriculumSettingsFields' => 'Curriculum Settings Fields',
                'QuizSettingsFields' => 'Quiz Settings Fields',
                'LessonSettingsFields' => 'Lesson Settings Fields',
                'QuestionSettingsFields' => 'Question Settings Fields',
                'OrderSettingsFields' => 'Order Settings Fields',
                'WordPressCustomFields' => 'WordPress Custom Fields',
                'TermsandTaxonomies' => 'Terms and Taxonomies',
                'IsSerialized' => 'Is Serialized',
                'NoCustomFieldsFound' => 'No Custom Fields Found', 
                'MediaUploadFields' => 'Upload Media Zip',
                'UploadMedia' => 'Upload Media',
                'UploadedListofFiles' => 'Uploaded List of Files',
                'UploadedMediaFileLists' => 'Uploaded Media File Lists',
                'SavethismappingasTemplate' => 'Save this mapping as Template',
                'Save' => 'Save',
                'Doyouneedtoupdatethecurrentmapping' => 'Do you need to update the current mapping ?',
                'Savethecurrentmappingasnewtemplate' => 'Save the current mapping as new template',
                'Back' => 'Back',
                'Size' => 'Size',
                'MediaHandling' => 'Manage Media Uploads',
                'Downloadexternalimagestoyourmedia' => 'Download external images to your media',
                'ImageHandling' => 'Image Handling',
                'Usemediaimagesifalreadyavailable' => 'Use media images if already available',
                'Overwritetheavailableimages' => 'Overwrite the available images',
                'ImageSizes' => 'Image Sizes',
                'Thumbnail' => 'Thumbnail',
                'Medium' => 'Medium',
                'MediumLarge' => 'Medium Large',
                'Large' => 'Large',
                'Custom' => 'Custom',
                'Slug' => 'Slug',
                'Width' => 'Width',
                'Height' => 'Height',
                'SIMPLEMODE' => 'SIMPLE MODE',
                'CustomerReviews' => 'Customer Reviews',
                'WooCommerceReviews' =>'WooCommerce Reviews',
                'Buynow' => 'BUY NOW!',
                'Addons' => 'Addons',
                'Posts' => 'Posts',
                'Found' => 'Found',
                'Pages' => 'Pages',
                'CustomPosts' => 'Custom Posts',
                'PostCategories' => 'Post Categories',
                'PostTags' => 'Post Tags',
                'Comments' =>'Comments',
                'WooCommerceProducts' => 'WooCommerce Products',
                'Taxonomies' => 'Taxonomies',
                'PremiumVersion' => 'Premium Version',
                'ContactusforPresaleEnquiry' => 'Contact us for Presale Enquiry',
                'Thisfeatureisavailable' => 'This feature is available in',
                'UltimateCSVImporterPro'=> 'Ultimate CSV Importer Pro',
                'Exporterwithadvancedfilters' => 'Exporter with advanced filters',
                'Updateolderpostsfromsingleimport' => 'Import large files without timeout errors',
                'JetEngineMetaboxToolsetTypesACFproFreeandPodsFieldPostPluginsImporter'=> 'Import WooCommerce variable products, orders, etc.',
                'SEOPluginsDataImporterRankMathYoastandAllinOneSEO' => 'Schedule recurring imports via cron jobs',
                'WarningImportforsomedataaredisabledInstallandactivatebelowpluginsfirst'=> 'Warning: Some addons are missing, it is recommended to',
                'AIOWooCommerceImportSuit' => 'Import JetEngine, ACF, MetaBox, and other custom fields',
                'PostContentImageOption' => 'Post Content Image Option',
                'installactivate' => 'to install and activate now',
                'EnabletodeletetheitemsnotpresentinCSVXMLfile' => 'Enable to remove the elements that are not present in the CSV/XML file',
                'EnableSingleImportOption' => 'Enable Single Import/Export Option',
                'subcribe' => 'Subscribe',
                'DeletedatafromWordPress' => 'Delete WordPres data',
                'EnableSingleImport' => 'Enable Single Import/Export',
                'GetSupport' => 'Get Support',
                'SampleCSVXML' => 'Sample CSV&XML',
                'AutoScheduledBackups' => 'Automatically scheduled backups',
                'Scheduledexporthelpsbackupaseditabletextfileformat' => 'Scheduled export helps support as editable text file format at regular interval.',
                'ExportfeatureisdisabledInstallandactivatetheaddonpluginfirst'=>'Warning: the export function is disabled. Install and activate the additional plugin first',
                'Gotoaddons'=>'Go to addons',
                'toaform' => 'to support the forum',
                'Learnfrom' => 'Learn from our blog posts',
                'TechnicalDocumentation' => 'Technical Documentation',
                'Getsampleandexamplefiles' => 'Get sample and example files',
                'instruction' => '(No.of records per file)',
                'Createasupport' => 'Create a support topic here for help',
                'Exportfiltereddata' => 'Export filtered data',
                'CreateTopic' => 'Create Topic',
                'WPMLImporter' => 'Update existing records without duplicates',
                'DownloadPostContentExternalImagestoMedia' => 'Download Post Content External Images to Media',
                'Addcustomsizes' => 'Add custom sizes',
                'backupineditableformat' => 'backup copy in editable format',
                'Letsyougetonlyrequireddataw    iththedifferentadvancedfilters' => 'Allows you to get only the required data with the different advanced filters',
                'MediaSEOAdvancedOptions' => 'Media SEO & Advanced Options',
                'SetimageTitle' => 'Set image Title',
                'Backupin4differentfileformatslikeCSV' => 'Backup in 4 different file formats like CSV, XML, JSON, XLS.',        
                'SetimageCaption' => 'Set image Caption',
                'WooCommerceOrders' => 'WooCommerce Orders',
                'WooCommerceCoupons' => 'WooCommerce Coupons',
                'UltimateExporterPro' => 'Ultimate Exporter Pro',
                'SetimageAltText' => 'Set image Alt Text',
                'WooCommerceVariations' => 'WooCommerce Variations',
                'WooCommerceRefunds' => 'WooCommerce Refunds',
                'SetimageDescription' => 'Set image Description',
                'Changeimagefilenameto' => 'Change image file name to',
                'ImportconfigurationSection' => 'Import configuration Section',
                'EnablesafeprestateRollback' => 'Enable safe prestate RollBack(Backup before import)',
                'Backupbeforeimport' => 'Backup before import',
                'DoyouwanttoSWITCHONMaintenancemodewhileimport' => 'Do you want to SWITCH ON Maintenance mode while import',
                'Doyouwanttohandletheduplicateonexistingrecords' => 'Do you want to handle the duplicate on existing records',
                'Mentionthefieldswhichyouwanttohandleduplicates' => 'Mention the fields which you want to handle duplicates',
                'DoyouwanttoUpdateanexistingrecords' => 'Do you want to Update an existing records',
                'Updaterecordsbasedon' => 'Update records based on',
                'DoyouwanttoSchedulethisImport' => 'Do you want to Schedule this Import',
                'ScheduleDate' => 'Schedule Date',
                'ScheduleFrequency' => 'Schedule Frequency',
                'TimeZone' => 'Time Zone',
                'ScheduleTime' => 'Schedule Time',
                'Schedule' => 'Schedule',
                'Import' => 'Start Import',
                'Update' => 'Update',
                'Format' => 'Format',
                'OneTime' => 'OneTime',
                'Daily' => 'Daily',
                'Weekly' => 'Weekly',
                'Monthly' => 'Monthly',
                'Hourly' => 'Hourly',
                'Every30mins'=> 'Every 30 mins',
                'Every15mins' => 'Every 15 mins',
                'Every10mins' => 'Every 10 mins',
                'Every5mins' => 'Every 5 mins',
                'FileName' => 'File Name',
                'FileSize' => 'File Size',
                'Process' => 'Process',
                'Totalnoofrecords' => 'Total no of records',
                'CurrentProcessingRecord' => 'Current Processing Record',
                'RemainingRecord' => 'Remaining Record',
                'Completed' => 'Completed Successfully',
                'TimeElapsed' => 'Time Elapsed',
                'approximate' => 'approximate',
                'DownloadLog' => 'View Log',
                'NoRecord' => 'No Record',
                'UploadedCSVFileLists' => 'Uploaded CSV File Lists',
                'Hostname' => 'Host Name',
                'HostPort' => 'Host Port',
                'HostUsername' => 'Host Username',
                'HostPassword' => 'HostPassword',
                'HostPath' => 'HostPath',
                'DefaultPort' => 'Default Port',
                'FTPUsername' => 'FTP Username',
                'FTPPassword' => 'FTP Password',
                'ConnectionType' => 'Connection Type',
                'ImportersActivity' => 'Importers Activity',
                'ImportStatistics' => 'Import Statistics',
                'Dashboard_ImportsRun' => 'Imports Run',
                'Dashboard_RecordsCreated' => 'Records Created',
                'Dashboard_RecordsExported' => 'Records Exported',
                'Dashboard_AIAssists' => 'AI Assists',
                'Dashboard_TotalTemplates' => 'Total Templates',
                'Dashboard_SavedTemplates' => 'Saved mapping templates',
                'Dashboard_TotalInserted' => 'Total inserted records',
                'Dashboard_ExportVolume' => 'Export volume',
                'Dashboard_ValidationFixes' => 'Validation fixes',
                'Dashboard_AIDetail' => 'Auto / Corrections / Validation',
                'Dashboard_ImportVolume' => 'Import Volume',
                'Dashboard_Last12Months' => 'Last 12 Months',
                'Dashboard_MonthlyImports' => 'Monthly Imports',
                'Dashboard_NoChartData' => 'No import data available yet.',
                'Dashboard_ContentDistribution' => 'Content Distribution',
                'Dashboard_RecentActivity' => 'Recent Activity',
                'Dashboard_ViewAllLogs' => 'View All Logs',
                'Dashboard_Activity' => 'Activity',
                'Dashboard_Records' => 'Records',
                'Dashboard_QuickStats' => 'Quick Stats',
                'Dashboard_TotalImports' => 'Total Imports',
                'Dashboard_TotalExports' => 'Total Exports',
                'Dashboard_ScheduledJobs' => 'Scheduled Jobs',
                'Dashboard_FailedJobs' => 'Failed Jobs',
                'Dashboard_TemplatesSaved' => 'Templates Saved',
                'Dashboard_MediaImported' => 'Media Imported',
                'FileManager' => 'File Manager',
                'SmartSchedule' => 'Smart Schedule',
                'ScheduledExport' => 'Scheduled Export',
                'Templates' => 'Templates',
                'LogManager' => 'Log Manager',
                'NotSelectedAnyTab' => 'Not Selected Any Tab',
                'EventInfo' => 'Event Info',
                'EventDate' => 'Event Date',
                'EventStatus' => 'Event Status',
                'Actions' => 'Actions',
                'Date' => 'Date',
                'Purpose' => 'Purpose',
                'Revision' => 'Revision',
                'Select' => 'Select',
                'SelectField' => 'Select Field',
                'Inserted' => 'Inserted',
                'Updated' => 'Updated',
                'Skipped' => 'Skipped',
                'Delete' => 'Delete',
                'Failed' => 'Failed',
                'Noeventsfound' => 'No events found',
                'ScheduleInfo' => 'Schedule Info',
                'ScheduledDate' => 'Scheduled Date',
                'ScheduledTime' => 'Scheduled Time',
                'Youhavenotscheduledanyevent' => 'You haven’t scheduled any event',
                'Frequency' => 'Frequency',
                'Time' => 'Time',
                'EditSchedule' => 'Edit Schedule',
                'SaveChanges' => 'Save Changes',
                'TemplateInfo' => 'Template Info',
                'TemplateName' => 'Template Name',
                'Module' => 'Module',
                'Action' => 'Actions',
                'CreatedTime' => 'Created Time',
                'NoTemplateFound' => 'No Template Found',
                'Download' => 'Download',
                'NoLogRecordFound' => 'No Log Record Found',
                'GeneralSettings' => 'General Settings',
                'DatabaseOptimization' => 'Database Optimization',
                'SecurityandPerformance' => 'Security and Performance',
                'Documentation' => 'Documentation',
                'MediaReport' => 'Media Report',
                'DropTable' => 'Drop Table',
                'Ifenabledplugindeactivationwillremoveplugindatathiscannotberestored' => 'If enabled plugin deactivation will remove plugin data, this cannot be restored.',
                'Scheduledlogmails' => 'Scheduled log mails',
                'Enabletogetscheduledlogmails' => 'Enable to get scheduled log mails.',
                'Sendpasswordtouser' => 'Send password to user',
                'Enabletosendpasswordinformationthroughemail' => 'Enable to send password information through email.',
                'WoocommerceCustomattribute' => 'Woocommerce Custom attribute',
                'Enablestoregisterwoocommercecustomattribute' => 'Enables to register woocommerce custom attribute.',
                'PleasemakesurethatyoutakenecessarybackupbeforeproceedingwithdatabaseoptimizationThedatalostcantbereverted' => 'Please make sure that you take necessary backup before proceeding with database optimization. The data lost cannot be reverted.',
                'DeleteallorphanedPostPageMeta' => 'Delete all orphaned Post/Page Meta',
                'Deleteallunassignedtags' => 'Delete all unassigned tags',
                'DeleteallPostPagerevisions' => 'Delete all Post/Page revisions',
                'DeleteallautodraftedPostPage' => 'Delete all auto drafted Post/Page',
                'DeleteallPostPageintrash' => 'Delete all Post/Page in trash',
                'DeleteallCommentsintrash' => 'Delete all Comments in trash',
                'DeleteallUnapprovedComments' => 'Delete all Unapproved Comments',
                'DeleteallPingbackComments' => 'Delete all Pingback Comments',
                'DeleteallTrackbackComments' => 'Delete all Trackback Comments',
                'DeleteallSpamComments' => 'Delete all Spam Comments',
                'RunDBOptimizer' => 'Run DB Optimizer',
                'Hire_us' => 'Hire us',
                'DatabaseOptimizationLog' => 'Database Optimization Log',
                'noofOrphanedPostPagemetahasbeenremoved' => 'no of Orphaned Post/Page meta has been removed.',
                'noofUnassignedtagshasbeenremoved' => 'no of Unassigned tags has been removed.',
                'noofPostPagerevisionhasbeenremoved' => 'no of Post/Page revisions has been removed.',
                'noofAutodraftedPostPagehasbeenremoved' => 'no of Auto drafted Post/Page has been removed.',
                'noofPostPageintrashhasbeenremoved' => 'no of Post/Page in trash has been removed.',
                'noofSpamcommentshasbeenremoved' => 'no of Spam comments has been removed.',
                'noofCommentsintrashhasbeenremoved' => 'no of Comments in trash has been removed.',
                'noofUnapprovedcommentshasbeenremoved' => 'no of Unapproved comments has been removed.',
                'noofPingbackcommentshasbeenremoved' => 'no of Pingback comments has been removed.',
                'noofTrackbackcommentshasbeenremoved' => 'no of Trackback comments has been removed.',
                'Allowauthorseditorstoimport' => 'Allow authors/editors to import',
                'Thisenablesauthorseditorstoimport' => 'This enables authors/editors to import.',
                'MinimumrequiredphpinivaluesIniconfiguredvalues' => 'Minimum required php.ini values (Ini configured values)',
                'Variables' => 'Variables',
                'SystemValues' => 'System Values',
                'MinimumRequirements' => 'Minimum Requirements',
                'RequiredtoenabledisableLoadersExtentionsandmodules' => 'Required to enable/disable Loaders, Extentions and modules:',
                'DebugInformation' => 'Debug Information:',
                'SmackcodersGuidelines' => 'Smackcoders Guidelines',
                'DevelopmentNews' => 'Development News',
                'WhatsNew' => 'Whats New?',
                'YoutubeChannel' => 'Youtube Channel',
                'OtherWordPressPlugins' => 'Other WordPress Plugins',
                'Count' => 'Count',
                'ImageType' => 'Image Type',
                'Status' => 'Status',
                'Loading' => 'Loading',
                'LoveWPUltimateCSVImporterGivea5starreviewon' => 'Love WP Ultimate CSV Importer, Give a 5 star review on',
                'ContactSupport' => 'Contact Support',
                'Email' => 'Email',
                'Supporttype' => 'Support type',
                'BugReporting' => 'Bug Reporting',
                'FeatureEnhancement' => 'Feature Enhancement',
                'Message' => 'Message',
                'Send' => 'Send',
                'NewsletterSubscription' => 'Newsletter Subscription',
                'Subscribe' => 'Subscribe',
                'Note' => 'Note',
                'SubscribetoSmackcodersMailinglistafewmessagesayear' => 'Subscribe to Smackcoders Mailing list (a few messages a year)',
                'Pleasedraftamailto' => 'Please draft a mail to',
                'Ifyoudoesnotgetanyacknowledgementwithinanhour' => 'If you does not get any acknowledgement within an hour!',
                'Selectyourmoduletoexportthedata' => 'Select the module to Export Data',
                'Toexportdatabasedonthefilters' => 'To export data based on the filters',
                'ExportFileName' => 'Export File Name',
                'AdvancedSettings' => 'Advanced Settings',
                'ExportType' => 'Export Type',
                'SplittheRecord' => 'Split the export into every',
                'AdvancedFilters'=> 'Advanced Filters',
                'Exportdatawithautodelimiters' => 'Export data with auto delimiters',
                'Delimiters' => 'Delimiters',
                'OtherDelimiters' => 'Other Delimiters',
                'Exportdataforthespecificperiod' => 'Export data for the specific period',
                'StartFrom' => 'Start From',
                'EndTo' => 'End To',
                'Exportdatawiththespecificstatus' => 'Export data with the specific status',
                'All' => 'All',
                'Publish' => 'Publish',
                'Sticky' => 'Sticky',
                'Private' => 'Private',
                'Protected' => 'Protected',
                'Draft' => 'Draft',
                'Pending' => 'Pending',
                'Exportdatabyspecificauthors' => 'Export data by specific authors',
                'Authors' => 'Authors',
                'ExportdatabasedonspecificInclusions' => 'Export data based on specific Inclusions',
                'DoyouwanttoSchedulethisExport' => 'Do you want to Schedule this Export',
                'SelectTimeZone' => 'Select TimeZone',
                'ScheduleExport' => 'Schedule Export',
                'DataExported' => 'Data Exported',
                'FilePath' => 'File Path',
                'WPUltimateCSVImporter' => 'WP Ultimate CSV Importer',
                'importwoocommerce' => 'import woocommerce',
                'ImportanybulkWooCommerceProductsdatainCSV' => 'Import any bulk WooCommerce Products data in CSV',
                'Highlights' => 'Highlights',
                'ProductTypessimplegroupedvariableexternaltypeimport' => 'Product Types simple, grouped, variable, external type import',
                'FeaturedProductImportfromURL' => 'Featured Product Import from URL',
                'Galleryimageimport' => 'Gallery image import',
                'Duplicatedetection' => 'Duplicate detection',
                'FileType' => 'File Type',
                'SupportsUTF_8CSVfile' => 'Supports UTF-8 CSV file',
                'AlreadyInstalled' => 'Already Installed',
                'Install' => 'Install',
                'ImportUsers' => 'Import Users',
                'ImportUserinfointoWordPressinbulk' => 'Import User info into WordPress in bulk',
                'WPMembersaddonsupport' => 'WP-Members add-on support',
                'Defaultcustomfieldsimport' => 'Default custom fields import',
                'Sendsautomatedpasswordnotificationemailoptional' => 'Sends automated password notification email(optional)',
                'WPUltimateExporter' => 'WP Ultimate Exporter',
                'ExportallyourWordPressdataasCSVfileforbackup' => 'Export all your WordPress data as CSV file for backup',
                'Supportsdefaultcustomfields' => 'Supports default custom fields',
                'UTF8encodedCSVfile' => 'UTF-8 encoded CSV file',
                'SupportPostPageCustomPost' => 'Support Post, Page & Custom Post',
                'Filteredexportbasedonperiodoftimeauthors' => 'Filtered export based on period of time & authors',
                'Users' => 'Users',
                'PleaseinstalltheUltimateExportertoexportallyourWordPressdataasCSV' => 'Please install the Ultimate Exporter to export all your WordPress data as CSV',
                'Clickheretoinstall' => 'Click here to install',
                'PluginVersion' => 'Plugin Version',
                'LifterCourseSettingsFields' => 'Lifter Course Settings Fields',
                'LifterReviewSettingsFields' => 'Lifter Review Settings Fields',
                'LifterCouponSettingsFields' => 'Lifter Coupon Settings Fields',
                'LifterLessonSettingsFields' => 'Lifter Lesson Settings Fields',
                'FifuPostFields' => 'Fifu Post Fields',
                'FifuPageFields' => 'Fifu Page Fields',
                'FifuCustomPostFields' => 'Fifu Custom Post Fields',
                'polylangfields' => 'Polylang Settings Fields',
                'BuddyFields' => 'BuddyPress Fields',
                'WPCompleteFields' => 'WPComplete Fields',
                "IterationId" => 'Interation Limit',
                'Iteration' => 'Iteration',
                'Exportiteration_id' => 'Export Processing in Iterations',
                'ImportCompleted' => 'Import Completed!',
                'importHasFinished' => 'Your import has finished successfully. Please click the button below to download & access a detailed import log.',
                'ImportLog' => 'Import Log',
                'FailedMedia' => 'Failed Media',
                'UseTheFailedImages' => 'Use the failed images CSV to correct URLs and re-import the images',
                'Toast_PleaseChooseHeaderManipulation' => 'Please choose Header Manipulation in the drop down',
                'Toast_ConfigureAISettingsConnectors' => 'Configure AI at Settings → Connectors.',
                'Toast_NoImagesInZip' => 'There is no image files in the uploaded zip file',
                'Toast_MandatoryField' => '{field} is a mandatory field',
                'Toast_UpgradeToProFeature' => 'Please upgrade to PRO for this feature',
                'Toast_SettingsUpdated' => 'Settings successfully updated',
                'Toast_AdminOnlyOption' => 'Only admin can change this option',
                'Toast_MailSent' => 'Mail sent successfully',
                'Toast_NewsletterSubscribed' => 'Successfully subscribed to newsletter',
                'Toast_InvalidEmail' => 'The email you entered is not valid',
                'Toast_BucketSelected' => 'Bucket selected',
                'Toast_LogDoesNotExist' => "Log doesn't exist",
                'Toast_NoFailedMediaRecords' => 'No failed media records found for this import.',
                'Toast_NoFailedMediaLog' => 'No failed media log available for this import.',
                'Toast_SelectModuleToExport' => 'Select a module to export',
                'Toast_FilePathMissing' => 'File path is missing!',
                'Toast_TypeExportFileName' => 'Please type export file name',
                'Support_RateUsPrefix' => 'Enjoying WP Ultimate CSV Importer? Rate us on',
                'Support_RateUsLinkText' => 'wordpress.org!',
                'Support_NeedHelp' => 'Need Help?',
                'Support_ThanksForUsing' => 'Thanks for using our plugin!',
                'Support_FreeSupportPrefix' => "If you're using the free version, please create a support topic on our",
                'Support_ForumLinkText' => 'WordPress.org support forum',
                'Support_ForumSuffix' => '. Our team actively monitors the forum and will be happy to assist you there.',
                'Support_PreSalePrefix' => 'Have a pre-sale question or thinking about upgrading to Pro? Feel free to email us directly at',
                'Support_PreSaleSuffix' => ", we’re here to help you choose the right solution.",
                'Support_ThanksAgain' => 'Thanks again for choosing Smackcoders!',
                'Support_SubmitQuery' => 'Submit query',
                'Banner_MaintenanceEnabled' => 'Maintenance mode is enabled',
                'Common_Disable' => 'Disable',
                'ImportResumeDetected' => 'An unfinished import was detected. Would you like to resume it?',
                'ImportResumeFile' => 'File',
                'ImportResumeProgress' => 'Progress',
                'ImportResumeButton' => 'Resume',
                'ImportResumeDiscard' => 'Discard',
                'ImportResumeDiscarding' => 'Discarding…',
                'ImportResumePreparing' => 'Preparing resume…',
                'ImportResumeFailed' => 'Could not resume this import.',
                'ImportResumeDiscardFailed' => 'Could not discard this import session.',
                'Banner_UpgradeMessage' => 'Unlock custom field imports, scheduled automation, bulk updates, and faster import/export with WP Ultimate CSV Importer Pro. Get 20% off automatically at checkout.',
                'Common_UpgradeToPro' => 'Upgrade to Pro',
                'Modal_ProOnlyMessage' => 'This feature is available in the Pro version only. Upgrade now to access!',
                'Help_ButtonLabel' => 'Help',
                'Help_MenuTitle' => 'Help Menu',
                'Help_FetchingData' => 'Fetching Data...',
                'Help_NoResults' => 'No results found',
                'Help_SearchPlaceholder' => 'Search here...',
                'Common_NeedHelp' => 'Need help?',
                'Month_January' => 'January',
                'Month_February' => 'February',
                'Month_March' => 'March',
                'Month_April' => 'April',
                'Month_May' => 'May',
                'Month_June' => 'June',
                'Month_July' => 'July',
                'Month_August' => 'August',
                'Month_September' => 'September',
                'Month_October' => 'October',
                'Month_November' => 'November',
                'Month_December' => 'December',
                'Toast_SelectFileToUpload' => 'Please select a file to upload.',
                'Toast_MissingRequiredFields' => 'Some required fields are missing. Please check the file and try again.',
                'Toast_FailedImportTryAgain' => 'Failed to import CSV. Please try again.',
                'Toast_ErrorOccurredTryAgain' => 'An error occurred while importing. Please try again.',
                'Toast_InstallPhpZip' => 'Please install php-zip extension',
                'Error_ServerTimeout' => 'Server connection has timed out',
                'Link_FixThisError' => 'Click here to fix this error',
                'Toast_UpdateUrlToProcess' => 'Please update URL to process',
                'Label_MatchBy' => 'Match by:',
                'Toast_SelectMatchField' => 'Please select a match field.',
                'Toast_MatchFieldNotMapped' => '{field} field is not mapped in the mapping section. Please map it in WordPress Core Fields.',
                'Common_AdminOnly' => 'This is only for administrator',
                'Common_FieldSettings' => 'Field Settings',
                'Common_ContentGenerator' => 'Content Generator',
                'Common_CustomFunction' => 'Custom Function',
                'Common_SearchField' => 'Search field...',
                'Common_HeaderManipulation' => 'Header Manipulation',
                'Common_ChooseCsvFile' => 'Choose CSV File',
                'Common_ImportConfigurationSection' => 'Import configuration Section',
                'Common_ProFeatureTooltip' => 'This feature is available in the Pro version',
                'MediaImportPath' => 'Media Import Path',
                'MediaImportPath_RetainOriginal' => 'Retaining the original file path instead of current month folder',
                'Module_WooCommerceReviews' => 'WooCommerce Reviews',
                'Module_WooCommerceProducts' => 'WooCommerce Products',
                'Module_WooCommerceOrders' => 'WooCommerce Orders',
                'Module_WooCommerceCustomers' => 'WooCommerce Customers',
                'Common_Images' => 'Images',
                'Common_Data' => 'Data',
                'Common_PRO' => 'PRO',
                'Common_SupportedFileTypes' => 'Supported file types',
                'Common_UploadTemplates' => 'Upload Templates',
                'Common_SmartSchedule' => 'Smart Schedule',
                'Common_ScheduleExport' => 'Schedule Export',
                'Common_GoogleSheet' => 'Google Sheet',
                'Common_DragDropFilesFor' => 'Drag & Drop your files for',
                'Pro_UpsellLong' => 'Unlock the full potential of this feature with our PRO version! Upgrade your experience and access exclusive benefits now.',
                'Pro_UpgradeNow' => 'Upgrade to PRO Now!',
                'Label_WhenDuplicateFound' => 'When duplicate found:',
                'DuplicateAction_Skip' => 'Skip',
                'DuplicateAction_UpdateExisting' => 'Update existing',
                'DuplicateAction_CreateNew' => 'Create new',
                'ImportConfig_NoCreateNewIfNoMatch' => 'Do not create new posts if no matching record is found',
                'ImportConfig_AdaptiveFileProcessing' => 'Do you want adaptive file processing of record',
                'ImportConfig_IterativePieceByPiece' => 'Iterative, Piece-by-Piece Processing',
                'ImportConfig_RecordsPerIteration' => 'In each iteration how many records need to process',
                'ImportConfig_FilterByCategory' => 'Do you want to import data filtered by category?',
                'Mapping_CreateWpCustomFields' => 'Create WP Custom Fields',
                'Mapping_ManageFilteringOptions' => 'Manage Filtering Options',
                'Mapping_AddCustomField' => 'ADD CUSTOM FIELD',
                'Mapping_AddRule' => 'ADD RULE',
                'Mapping_NoWpCustomFields' => 'No WP Custom Fields',
                'Mapping_ProductAttribute' => 'PRODUCT ATTRIBUTE {n}',
                'Common_Of' => 'of',
                'Common_Add' => 'Add',
                'Filter_AND' => 'AND',
                'Filter_OR' => 'OR',
                'Export_WPQuery' => 'WP Query',
                'Export_QueryPostType' => 'Post Type Query',
                'Export_QueryUser' => 'User Query',
                'Export_QueryComment' => 'Comment Query',
                'Export_QueryArgsHint' => 'Enter your WP_Query arguments as a JSON-like object.',
                'Export_QueryArgsTooltipTitle' => 'You can enter arguments like:',
                'Export_GoogleSheetCsv' => 'Export CSV data to the Google Sheet',
                'Export_ClientMode' => 'Allow non-admins to run this export in Client Mode',
                'Export_ByLanguageCode' => 'Export data based on language code',
                'Export_ByPostId' => 'Export data by specific post ID',
                'Export_ByPostTitle' => 'Export data by specific post title',
                'Export_CustomersPurchased' => 'Export customers based on customer purchased',
                'Export_CustomersRole' => 'Export customers based on role',
                'Export_CustomersByProduct' => 'Export customers based on the product that the customer purchased',
                'Export_EnableRollback' => 'Enable Rollback',
                'Export_OrdersByProduct' => 'Export Orders based on the Product that the order is placed.',
                'Export_OrderByPayment' => 'Export Order based on Payment Options.',
                'Export_Bundle' => 'Bundle',
                'Export_DataPreview' => 'Data Preview',
                'Export_WithIteration' => 'Export with Iteration',
                'Export_EnterIterationId' => 'Enter Iteration ID',
                'Module_JetBooking' => 'Jet Booking',
                'Module_JetReviews' => 'Jet Reviews',
                'Export_SplitRecord' => 'Split Record',
                'Export_AvailableRecords' => 'Available records',
                'Export_ZeroRecordsHint' => 'If record count is zero, kindly change the filter settings and click Export.',
                'Common_Preview' => 'Preview',
                'Export_Migrate' => 'Migrate',
                'Feedback_Title' => 'How was your experience?',
                'Feedback_Subtitle' => 'Help us improve WP Ultimate CSV Importer',
                'Feedback_RateOnWpOrg' => 'Rate our plugin on WordPress.org',
                'Feedback_RatingHelps' => 'Your rating helps other users discover our plugin',
                'Feedback_RateButton' => 'Rate on WordPress.org',
                'Common_Or' => 'or',
                'Feedback_UnlockPremium' => 'Unlock Premium Features',
                'Feedback_FeatureMapping' => 'Advanced field mapping',
                'Feedback_FeatureScheduled' => 'Scheduled imports',
                'Feedback_FeatureCPT' => 'Custom post types support',
                'Feedback_FeatureSupport' => 'Priority support',
                'Feedback_UpgradePremium' => 'Upgrade to Premium',
                'Feedback_DontShowAgain' => "Don't Show Again",
                'Feedback_MaybeLater' => 'Maybe Later',
                'Mapping_AddedRules' => 'Added Rules:',
                'Mapping_DelimiterDetected' => 'Delimiter detected from your uploaded file :',
                'Common_SelectOption' => '--select--',
                'Common_SelectDash' => '-- Select --',
                'Common_CsvFields' => 'CSV Fields',
                'Filter_Elements' => 'Elements',
                'Filter_Rule' => 'Rule',
                'Filter_Value' => 'Value',
                'Filter_Rules' => 'Rules',
                'Filter_Values' => 'Values',
                'Filter_Condition' => 'Condition',
                'Filter_Actions' => 'Actions',
                'Filter_EnterValue' => 'Enter value',
                'FilterRule_Equals' => 'Equals',
                'FilterRule_NotEquals' => 'Not Equals',
                'FilterRule_GreaterThan' => 'Greater Than',
                'FilterRule_EqualsOrGreaterThan' => 'Equals or Greater Than',
                'FilterRule_LessThan' => 'Less Than',
                'FilterRule_EqualsOrLessThan' => 'Equals or Less Than',
                'FilterRule_Contains' => 'Contains',
                'FilterRule_NotContains' => 'Not Contains',
                'FilterRule_IsEmpty' => 'Is Empty',
                'FilterRule_IsNotEmpty' => 'Is Not Empty',
                'DelimiterName_Comma' => 'comma',
                'DelimiterName_Semicolon' => 'semicolon',
                'DelimiterName_Tab' => 'tab',
                'DelimiterName_Pipe' => 'pipe',
                'DelimiterName_Unknown' => 'unknown',
                'FieldLabel_Title' => 'Title',
                'FieldLabel_ID' => 'ID',
                'FieldLabel_Content' => 'Content',
                'FieldLabel_ShortDescription' => 'Short Description',
                'FieldLabel_PublishDate' => 'Publish Date',
                'FieldLabel_Slug' => 'Slug',
                'FieldLabel_Author' => 'Author',
                'FieldLabel_Status' => 'Status',
                'FieldLabel_FeaturedImage' => 'Featured Image',
                'FieldLabel_Format' => 'Format',
                'FieldLabel_CommentStatus' => 'Comment Status',
                'FieldLabel_PingStatus' => 'Ping Status',
                'FieldLabel_Parent' => 'Parent',
                'FieldLabel_Order' => 'Order',
                'FieldLabel_PageTemplate' => 'Page Template',
                'FieldLabel_LanguageCode' => 'Language Code',
                'FieldLabel_PostExpirator' => 'Post Expirator',
                'FieldLabel_PostExpiratorStatus' => 'Post Expirator Status',
                'FieldLabel_TemplateTitle' => 'Template title',
                'FieldLabel_TemplateContent' => 'Template content',
                'FieldLabel_Style' => 'Style',
                'FieldLabel_TemplateType' => 'Template type',
                'FieldLabel_CreatedTime' => 'Created time',
                'FieldLabel_CreatedBy' => 'Created by',
                'FieldLabel_TemplateStatus' => 'Template status',
                'FieldLabel_Category' => 'Category',
                'AlwaysCreateAsNewImage' => 'Always Create As New Image',
                'ChooseUploadMethod' => 'Choose Upload Method',
                'CsvUploadFields'=>'Csv Upload Fields',
                'Device' => 'Device',
                'Media' =>'Media',
                'Remote' => 'Remote',
                'SelectDeviceZIPfile' =>'Select Device to upload images directly from your Device as a ZIP file.',
                'SelectDeviceCSVfile' => 'Choose Remote to import images from Remote website URLs.',
                'MediaContinue' => 'Continue',
                'FreshImport' => 'Fresh import',
                'UpdateContent' => 'Content Update',
                'UpdateThisMappingAs' => 'Update this mapping as',
                'Summary' => 'Summary',
                'Validation_ValidateCsv' => 'Validate CSV',
                'Validation_ResultsTitle' => 'CSV Validation Results',
                'Validation_ScanMode' => 'Scan mode',
                'Validation_Quick10' => 'Quick scan (10 rows)',
                'Validation_Quick50' => 'Quick scan (50 rows)',
                'Validation_FullScan' => 'Full scan',
                'Validation_HealthScore' => 'Health Score',
                'Validation_Warnings' => 'Warnings',
                'Validation_Critical' => 'Critical Errors',
                'Validation_CanImport' => 'Can Import',
                'Validation_ScannedSummary' => 'Scanned {scanned} of {total} rows.',
                'Validation_Row' => 'Row',
                'Validation_Column' => 'Column',
                'Validation_Field' => 'Field',
                'Validation_Severity' => 'Severity',
                'Validation_Message' => 'Message',
                'Validation_NoIssues' => 'No validation issues found.',
                'Common_Yes' => 'Yes',
                'Common_No' => 'No',
                'Common_Close' => 'Close',
                'Toast_ValidationBlocked' => 'Import is blocked due to critical validation errors.',
                'Toast_ValidationWarnings' => 'Validation completed with warnings. You can still import.',
                'Toast_ValidationPassed' => 'CSV validation passed.',
                'Toast_ValidationFailed' => 'CSV validation request failed.',

                );
        return $response;
        }
        public static function notice_contents()
        {
        $result =array(
                'UpgradetoPROusingcode' => 'Upgrade to PREMIUM using code',
                'Unlockfeatureslikebulkimportadvanced exportschedulingcontentupdatemorepluslifetimesupport'  =>'Unlock features like bulk import, advanced export, scheduling, content update, & more, plus lifetime support',
                'upgradenow' => 'upgrade now',
                        'UnlockProFeature' => 'Unlock Pro Version',
                        'Export_WPQueryTitle' => 'WP_Query Export',
                        'Export_QueryTypeLabel' => 'Select Query Type',
                        'Export_QueryArgsLabel' => 'WP_Query Arguments',
                        'Export_Available' => 'Available',
                        'Export_FileSettings' => 'File Settings',
                        'Export_Format' => 'Export Format',
                        'Export_SpecificPeriodDesc' => 'Filter exported items by date range',
                        'Export_IterationIdDesc' => 'Export items from a specific batch execution',
                        'Export_SpecificAuthorsDesc' => 'Filter exported items by selected authors',
                        'Export_SplitRecordDesc' => 'Export data into multiple files of custom size',
                        'Export_SpecificStatusDesc' => 'Export items with specific post status',
                        'Export_AutoDelimitersDesc' => 'Export data with customized auto separators',
                        'Export_SpecificInclusionsDesc' => 'Export data based on selected column fields',
                        'Export_ScheduleDesc' => 'Schedule automatic exports at configured intervals',
                        'Export_GoogleSheetDesc' => 'Automatically push export results to a Google Sheet',
                        'Export_ClientModeDesc' => 'Authorize non-admin roles to execute this export',
                        'Export_ByLanguageDesc' => 'Filter multilingual posts/pages by language code',
                        'Export_ByPostIdDesc' => 'Export only selected posts/products by ID',
                        'Export_SpecificCategoryDesc' => 'Export records belonging to selected categories',
                        'Export_ByPostTitleDesc' => 'Search and export matching post/product titles',
                        'Export_CustomersPurchasedDesc' => 'Export customers with purchase records',
                        'Export_CustomersRoleDesc' => 'Export WordPress users based on user role',
                        'Export_CustomersByProductDesc' => 'Export users who purchased a specific product ID',
                        'Export_EnableRollbackDesc' => 'Allow rolling back to original user state after export',
                        'Export_OrdersByProductDesc' => 'Export orders matching a specific product ID',
                        'Export_OrderByPaymentDesc' => 'Export orders matching selected gateway methods',
                        'ToastTemplateLoadFailed' => 'Unable to load template. Please try again.',
                        'ToastTemplateDeleted' => 'Template Deleted Successfully',
                        'ToastSelectRevision' => 'Please Select any Revision',
                        'AreYouSure' => 'Are you sure?',
                        'ConfirmDeleteTemplate' => 'You want to delete this template?',
                        'NotNow' => 'Not Now',
                        'AutomationLoadError' => 'Could not load API settings.',
                        'SaveAutomationSettings' => 'Save automation settings',
                        'Copied' => 'Copied',
                        'CopyTemplateId' => 'Copy ID',
                        'SearchTemplatesOrModules' => 'Search templates or modules...',
                        'TemplatesCount' => 'Templates',
                        'TemplateId' => 'Template ID',
                        'AutomationTemplateSettings' => 'API / Webhook Settings',
                        'TemplateIdHelp' => 'Use this ID in API and webhook URLs.',
                        'AutomationApiTriggerUrl' => 'API trigger URL',
                        'AutomationWebhookUrl' => 'Webhook URL',
                        'EnableApiTrigger' => 'Enable API trigger',
                        'EnableWebhook' => 'Enable webhook',
                        'WebhookSecret' => 'Webhook secret',
                        'AllowedIps' => 'Allowed IPs',
                        'ToastInstallPhpZip' => 'Please install php-zip extension',
                        'UploadTemplate' => 'Upload Template',
                        'Import_AdvancedOptions' => 'Advanced Options',
                        'Common_Cancel' => 'Cancel',
                        'Validation_Status' => 'Validation Status',
                        'Media_UseExistingDesc' => 'Use existing media images if they are already available in the media library',
                        'Media_OverwriteDesc' => 'Replace existing images with new ones during import',
                        'Media_AlwaysCreateDesc' => 'Always upload and create a new image record',
                        'Media_PostContentDesc' => 'Download images referenced in post content to local media library',
                        'ToastTemplateSaved' => 'Template saved successfully',
                        'ToastChooseAnotherName' => 'Please Choose Another Name',
                        'ToastSaveFailed' => 'Save failed',
                        'SaveMappingAs' => 'Save this mapping as',
                        'NeedHelp' => 'Need a Help?',
                        'Searching' => 'Searching...',
                        'ClearSearch' => 'Clear Search',
                        'Prev' => 'Prev',
                        'Next' => 'Next',
                        'Dashboard_ImportVolumeSubtitle' => 'Records imported per {periodLabelText}, all sources combined.',
                        'There' => 'there',
                        'Dashboard_NoCompletedRecordVolume' => 'No completed record volume yet. Start an import to see trends here.',
                        'Dashboard_SameCompletedVolume' => 'About the same completed volume as the previous period.',
                        'YouImported' => 'You imported',
                        'Records' => 'records',
                        'InTotal' => 'in total.',
                        'Dashboard_StartNewImport' => 'Start a new import',
                        'Dashboard_BrowseTemplates' => 'Browse templates',
                        'Dashboard_RecordsTotal' => 'records total',
                        'Dashboard_JumpInImports' => 'That\'s a {growthPercent}% jump in imports — your data synchronization is doing great!',
                        'Dashboard_DecreaseInImports' => 'That\'s a {growthPercent}% decrease in completed record volume compared to the previous period.',
                        'Dashboard_ActivityLogs' => 'Latest data import activities',
                        'Dashboard_RecordTypeShare' => 'Distribution across records',
                        'Dashboard_NoDistributionData' => 'No record distribution data yet.',
                        'WooCommerceProduct' => 'WooCommerce Product',
                        'COMPLETED' => 'COMPLETED',
                        'PROCESSING' => 'PROCESSING',
                        'FAILED' => 'FAILED',
                        'SCHEDULED' => 'SCHEDULED',
                        'GoodMorning' => 'Good morning',
                        'GoodAfternoon' => 'Good afternoon',
                        'GoodEvening' => 'Good evening',
                        'Manage' => 'Manage',
                        'AuditlogsandTools' => 'Audit logs and Tools',
                        'loginfo' => 'Log Info',
        );
        return $result;
        }
}

